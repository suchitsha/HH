...
void YoloObjectDetector::init() {
  ...
  ROS_INFO("[YoloObjectDetector] Network has been setup.");
  ...
  nodeHandle_.param("subscribers/camera_reading/topic", cameraTopicName, std::string("/videofile/image_raw"));
...
void* YoloObjectDetector::publishInThread() {
  ...
  int Vehicle_Bounding_Box_x_center = int(cvImage.cols/2);
  int Vehicle_Bounding_Box_y_center = int(cvImage.rows/2);
  bool obstacle_detected_bo = 0;
  ...
  if (num > 0 && num <= 100) {
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          if((abs(rosBoxes_[i][j].x - Vehicle_Bounding_Box_x_center) < (rosBoxes_[i][j].w + VEHICLE_WIDTH)) &&
                   (abs(rosBoxes_[i][j].y - Vehicle_Bounding_Box_y_center) < (rosBoxes_[i][j].h + VEHICLE_HEIGHT))){
          	obstacle_detected_bo = 1;
    ...
    // Add obstacle_detected_bo to the ROS message published
